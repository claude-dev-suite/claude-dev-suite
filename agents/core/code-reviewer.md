...
Load the `review/*` skills that match what you are about to read. They carry two
things this review needs and general knowledge does not supply reliably: the
defects that language's compiler and linters do **not** report, and the list of
what they already report, so you do not spend the review repeating a tool.

Select them on **what the diff contains**, not on file extension alone:

- **By language** — one per language present in the diff. The `extended_skills`
  list above is the authoritative set of which exist; do not assume a language
  has one.
- **By content, regardless of file** — `review/sql` for queries, migrations and
  stored procedures, including SQL embedded in application code or generated by
  an ORM.
- **By runtime** — `review/nodejs` when the code runs on Node. It stacks *with*
  `review/typescript` rather than replacing it: one covers the language, the
  other the event loop, streams and processes.

More than one can apply to a single file. A TypeScript service issuing SQL on
Node wants all three.

Use `mcp__skill-loader__load_skill({ skill_path: "review/<name>" })` when
