...
/**
 * SSRF protection utilities for the performance-profiler MCP server.
 *
 * Policy:
 ...
 *    by setting PERF_PROFILER_ALLOW_PRIVATE_URLS=1.
...
function getBlockedIpv4Range(ip: string): string | null {
  ...
  // Allow remaining ranges when PERF_PROFILER_ALLOW_PRIVATE_URLS=1
  if (process.env['PERF_PROFILER_ALLOW_PRIVATE_URLS'] === '1') return null;
...
function getBlockedIpv6Range(addr: string): string | null {
  ...
  const allowPrivate = process.env['PERF_PROFILER_ALLOW_PRIVATE_URLS'] === '1';
  ...
  if (isIpv4Mapped) {
    ...
    const blocked = getBlockedIpv4Range(embeddedIpv4);
...
export async function validateUrl(rawUrl: string): Promise<void> {
  ...
  if (isIPv6(stripped) || stripped.includes(':')) {
    const reason = getBlockedIpv6Range(stripped);
    if (reason) {
  ...
  if (isIPv4(stripped)) {
    const blockedRange = getBlockedIpv4Range(stripped);
    if (blockedRange) {
      throw new Error(
  ...
  if (decoded !== null) {
    ...
    const blockedRange = getBlockedIpv4Range(decoded);
    if (blockedRange) {
      throw new Error(
  ...
  for (const addr of addresses) {
    if (isIPv6(addr) || addr.includes(':')) {
      const reason = getBlockedIpv6Range(addr);
      if (reason) {
        throw new Error(
      ...
      const blockedRange = getBlockedIpv4Range(addr);
      if (blockedRange) {
        throw new Error(
