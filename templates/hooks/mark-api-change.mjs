...
/** Compiled once, from the constant above — never from anything external. */
const API_PATTERN = new RegExp(DEFAULT_API_PATTERN, 'i');
...
async function main() {
  ...
  // The built-in pattern is a constant compiled from this file. The override is
  // a comma-separated list of substrings rather than a regex: an environment
  // variable is an unbounded external string, and compiling one on every file
  // write is a sink worth not having.
  const overrides = (process.env.DS_API_SURFACE_CONTAINS ?? '')
    .split(',')
    .map(part => part.trim().toLowerCase())
    .filter(Boolean);
  const matched = overrides.length > 0
    ? overrides.some(needle => normalized.toLowerCase().includes(needle))
    : API_PATTERN.test(normalized);
  if (!matched) return;
