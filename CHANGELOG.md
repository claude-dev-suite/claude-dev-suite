...
- **`code-reviewer` selects review skills by what the diff contains**, not by
  file extension. The instruction was written when `review/go` was the only such
  skill and said "`review/go` for Go, and so on" — which does not describe two of
  the eleven that now exist. `review/sql` applies to queries wherever they live,
  including SQL embedded in application code or generated by an ORM, and
  `review/nodejs` stacks *with* `review/typescript` rather than replacing it,
  since one covers the language and the other the event loop. The body no longer
  enumerates the languages: `extended_skills` is the authoritative list, and
  duplicating it there was one more place to go stale.
...
- **An `-rc` tag published as a stable release, and the updater offered it.**
  `release.yml` called `gh release create` with no `--prerelease`, so
  `v1.14.0-rc.1` became the repository's "Latest" release, and anyone on 1.13.0
  would have been offered an rc as a stable update.

  `autoUpdater.allowPrerelease = false` is set (`electron/updater.cjs`) and does
  **not** protect against this. With it false, `GitHubProvider` calls
  `getLatestTagName()`, which for github.com requests `/releases/latest` — and
  GitHub excludes from that endpoint only the releases *flagged* as
  pre-releases. The judgement is delegated entirely to that flag; nothing in
  that path parses the semver pre-release component of the tag. An unflagged
  `-rc.1` is therefore indistinguishable from a stable release to the updater.

  The workflow now flags any tag containing a hyphen (semver's pre-release
  marker) and, for robustness, re-flags an existing release created before this
  guard. `v1.14.0-rc.1` has been marked retroactively; `v1.13.0` is "Latest"
  again.

## [1.14.0] - 2026-09-05

> Shipped only as `v1.14.0-rc.1`, whose purpose was validating the Node 22
> release pipeline on all three runners. Fifteen commits landed before it was
> promoted, so this content ships inside 1.15.0 rather than under its own
> stable tag. The section is kept as written rather than folded upward: a
> dated entry rewritten to absorb later work is how a changelog stops being
> trustworthy.
